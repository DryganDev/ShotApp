    ...
    @IBAction func buttonTapped(_ sender: NSButton) {
        do {
            ...
            if (FileManager.default.fileExists(atPath: destinationImage.path)) {
                ...
                guard let imageData = bits.representation(using: .jpeg, properties: [.compressionFactor: 0.9]) else { return }
                Alamofire.upload(
                    multipartFormData: { multipartData in
                        multipartData.append(imageData, withName: "content", fileName: "file", mimeType: "image/jpeg")
                    },
